...
const BV_PASSKEY = 'caOGkxt5ZGxRUy0oZU3zbSlV36IBwxAijWghipc2FSoQY';

/**
 * Extract the BazaarVoice product ID from a Lululemon URL.
 * URL pattern: /p/category/Product-Name-Here/_/prod12345
 * BV ID format: Product_Name_Here (hyphens → underscores)
 */
function extractBVProductId(url: string): string {
  const path = new URL(url).pathname;
  // Match the product name segment: /p/<category>/<ProductName>/...
  const match = path.match(/\/p\/[^/]+\/([^/]+)/);
  if (!match?.[1]) return '';
  // Convert hyphens to underscores to match BV product ID format
  return match[1].replace(/-/g, '_');
}

async function fetchLululemonReviews(
  productId: string,
  limit = Infinity,
  ...
  while (reviews.length < limit) {
    ...
    if (!response.ok) {
      throw new Error(`Lululemon BazaarVoice API error: ${response.status}`);
    }
...
export const lululemonScraper: ReviewScraper = {
  name: 'lululemon',

  canHandle(url: string): boolean {
    try {
      const hostname = new URL(url).hostname.toLowerCase();
      return hostname.includes('lululemon.com');
    } catch {
      return false;
    }
  },
  ...
  async scrape(url: string, onBatch?: BatchCallback): Promise<ScrapeResult> {
    const productId = extractBVProductId(url);
    if (!productId) {
      throw new Error('Could not extract product ID from Lululemon URL');
    }

    console.log(`  Lululemon BV Product ID: ${productId}`);

    const { reviews, product } = await fetchLululemonReviews(productId, Infinity, onBatch);

    if (reviews.length === 0) {
      throw new Error(
        'No reviews found for this Lululemon product. The product may be too new or the product ID may be incorrect.'
      );
    }
