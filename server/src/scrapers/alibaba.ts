...
/**
 * Alibaba scraper – extracts reviews/feedback from Alibaba product pages.
 * Alibaba is a B2B marketplace with supplier feedback and product reviews.
 *
...
const EXTRACT_PAGE_META_JS = `(() => {
  ...
  // Title
  var titleEl = document.querySelector('.product-title, h1[class*="title"], h1');
  ...
  // Image
  var imgEl = document.querySelector('.detail-gallery-image img, .main-image img, [class*="product-img"] img');
  ...
  // Rating
  var ratingEl = document.querySelector('[class*="score"], [class*="rating-value"], [class*="star-score"]');
  if (ratingEl) {
  ...
  // Review count
  var countEl = document.querySelector('[class*="review-count"], [class*="reviews-num"], [class*="feedback-num"]');
  if (countEl) {
...
const CLICK_REVIEWS_TAB_JS = `(() => {
  // Click the "Reviews" or "Feedback" tab
  var tabs = document.querySelectorAll('[class*="tab"], [role="tab"], a[href*="review"], a[href*="feedback"]');
  for (var tab of tabs) {
  ...
  var section = document.querySelector('[id*="review"], [id*="feedback"], [class*="review-list"]');
  if (section) {
...
const EXTRACT_REVIEWS_JS = `(() => {
  ...
  // Alibaba review items
  var reviewEls = document.querySelectorAll(
    '[class*="review-item"], [class*="feedback-item"], ' +
    '[class*="review-list"] > div, [class*="review-list"] > li, ' +
    '.customer-review, [class*="buyer-review"]'
  );
  ...
  for (var el of reviewEls) {
    ...
    // Rating from stars
    var starEl = el.querySelector('[class*="star"], [class*="rating"]');
    if (starEl) {
    ...
    if (!rating) {
      var filled = el.querySelectorAll('[class*="star"][class*="full"], [class*="star"][class*="active"]');
      if (filled.length > 0 && filled.length <= 5) rating = filled.length;
    }
    if (!rating) {
      var ratingText = el.querySelector('[class*="score"], [class*="rating-value"]');
      if (ratingText) {
    ...
    var bodyEl = el.querySelector(
      ...
      '[class*="review-text"], [class*="comment"]'
    );
    ...
    // Date
    var dateEl = el.querySelector('[class*="date"], [class*="time"], time');
    if (dateEl) {
...
const CLICK_NEXT_PAGE_JS = `(() => {
  var nextBtn = document.querySelector(
    ...
    'a[class*="next-page"]:not([class*="disabled"])'
  );
  if (nextBtn) {
    nextBtn.click();
    return true;
...
export const alibabaScraper: ReviewScraper = {
  name: 'alibaba',
  ...
  async scrape(url: string, onBatch?: BatchCallback): Promise<ScrapeResult> {
    ...
    try {
      console.log('  Alibaba: Loading product page...');
      ...
      console.log(`  Alibaba: Product "${pageMeta.title}", ${pageMeta.reviewCount} reviews`);
      ...
      console.log('  Alibaba: Reviews tab:', tabResult);
      await delay(3000);
      ...
      if (allReviews.length === 0) {
        throw new Error(
          'No reviews found for this Alibaba product. The page may not have reviews or requires login.'
        );
      }

      console.log(`  Alibaba: Fetched ${allReviews.length} reviews`);
